...
This cheat sheet is a fork of [the Progfun wiki](https://github.com/lampepfl/progfun-wiki) which in turn originated from the forum of [Functional Programming Principles in Scala](https://www.coursera.org/learn/progfun1), credits to [Laurent Poulain](https://www.linkedin.com/in/laurentpoulain/).
...
comparison functions such as `lt()` and `gt()` for standard types. Types with a single natural ordering should inherit
from the trait `scala.math.Ordered[T]`.
```scala
